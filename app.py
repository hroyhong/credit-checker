...
# Dynamically generated based on 卓院班 + 英院班 selection
# ============================================================

def build_requirements(zhuo_class, ying_class):
    """Build requirements dict based on class selection.

    zhuo_class: 高翻班 / 国别班 / 国组班 / 外交班
    ying_class: 英语 / 翻译 / 翻译+工管 / 英语+国政
    """
    ...
    # Platform courses differ by 卓院班
    if zhuo_class == "国别班":
        platform_category = {
...
def parse_transcript(text):
    """Parse pasted transcript text into structured course list."""
    courses = []
...
def categorize_course(course, ying_class, zhuo_class):
    """Determine which 培养方案 category a course belongs to."""
    code = course["code"]
    name = course["name"]
    ...
    if code == "SRPP2113":
        return (zhuo_label, "卓越博雅课程 (14学分)", "博雅选修")
    if code.startswith("SRPP") or code.startswith("IOPP") or code.startswith("DFPP") or code.startswith("ITPP"):
    ...
    # === 英语学院 ===
    if code.startswith("FFS.") or code.startswith("ELS.") or code.startswith("SGE."):
    ...
    if ying_class == "翻译+工管":
        ...
        # 手动修正：系统标错的课程
        OVERRIDE_TO_DIRECTION = {"TIBA4203"}  # 人工智能导论，实为方向课
...
def check_credits(courses, requirements, grand_total, ying_class, zhuo_class):
    """Cross-check courses against requirements."""
    credit_map = {}
    uncategorized = []
